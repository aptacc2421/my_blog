...
| 生活 | `life.html` | 随机一个栏目：字符画背景 + 该栏目片段。每个片段只显示**一行标题**（首行 `### 标题` 或首行正文）；**点击标题**进入 **`scene.html?cat=…&file=…`**，整篇 `.md` 用 Markdown 渲染。刷新重掷；左下 **回** → 门厅。**指定场景**：地址后加 **`?cat=栏目名`**（或 **`?world=`**），栏目名须与 `scenes/index.json` 里的 key **完全一致**。若**写错栏目名**，页面会提示错误且**不会**再随机进入其它世界（避免误以为参数无效）。若栏目存在但**文件列表为空**，会提示该栏目尚无 `.md`，而不是显示别的栏目的碎片。 |
| 片段阅读 | `scene.html` | 由生活页链接进入；参数 **`cat`**（栏目名）、**`file`**（文件名）。**回生活** 链回 `life.html`。 |
