...
| 生活 | `life.html` | 随机一个栏目：字符画背景 + 该栏目片段。每个片段只显示**一行标题**（首行 `### 标题` 或首行正文）；**点击标题**进入 **`scene.html?cat=…&file=…`**，整篇 `.md` 用 Markdown 渲染。刷新重掷；左下 **回** → 门厅。**指定场景**：地址后加 **`?cat=栏目名`**（或 **`?world=`**），栏目名须与 `scenes/index.json` 里的 key 完全一致，例如 `life.html?cat=无名之地`。写错或未收录的栏目名仍会随机。 |
